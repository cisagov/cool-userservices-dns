...
resource "aws_acm_certificate" "domainmanager_certificate" {
  provider = aws.userservices_domainmanager
...
resource "aws_route53_record" "domainmanager_certificate_validation" {
  ...
  for_each = {
    for dvo in aws_acm_certificate.domainmanager_certificate.domain_validation_options : dvo.domain_name => {
  ...
  depends_on = [aws_acm_certificate.domainmanager_certificate]
}
...
resource "aws_acm_certificate_validation" "domainmanager_certificate_validation" {
  provider = aws.userservices_domainmanager

  certificate_arn         = aws_acm_certificate.domainmanager_certificate.arn
